...
export const RecipesList = () => {
  ...
  if (loading) return <RecipeListLoading />;

  return (
    <div className='pb-3'>
      <div className='grid 2xl:grid-cols-3 md:grid-cols-2 sm: grid-cols-1 p-3 gap-x-3 gap-y-2'>
        {filteredRecipes
          .slice(currentPageIndex * limit, (currentPageIndex + 1) * limit)
          .map((recipe) => (
      ...
      <Pagination className='w-full flex justify-center items-center'>
        <PaginationContent>
          ...
          {[...Array(totalPages).keys()].map((pageIndex) => (
            <PaginationItem
              className={cn(
                'rounded-lg border-zinc-300 border',
                pageIndex === currentPageIndex &&
                  'border-sky-500 border text-sky-500',
              )}
              key={pageIndex}
            >
