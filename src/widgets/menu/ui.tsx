import { clearFilters, filterByCuisine, filterByDifficulty, filterByMealType, IRecipe, TRecipeDifficulty } from '@/entities/recipe';
...
const Menu = () => {
    const recipes: IRecipe[] = useSelector((state: RootState) => state.recipe.recipes)
    ...
    const types = [...new Set((recipes.map(recipe => recipe.mealType.map(item => item))).flat())];
    const [currentDifficulty, setCurrentDifficulty] = useState<TRecipeDifficulty | "">("")
    ...
    return (
        <Card className="p-6 h-full bg-white flex-col justify-start items-start gap-12 inline-flex">
            ...
            <div className="flex flex-col gap-4 w-full">
                ...
                <div className="flex gap-3 items-center w-full">
                    ...
                    <div className="flex">
                        {difficulties.map((difficulty: IDifficulty, index: number) => 
                            <Button onClick={() => handleChangeDifficulty(difficulty.difficulty)} className={cn(index === 0 && "rounded-l-sm", index === difficulties.length - 1 && "rounded-r-sm", )} size="sm" variant={currentDifficulty === difficulty.difficulty ? "default" : "outline"}>
                                {difficulty.name}
                            </Button>
                        )}
                    </div>
                ...
                <span onClickCapture={handleClear} className="text-sky-500/40 text-sm font-normal leading-snug">
